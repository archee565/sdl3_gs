...
use sdl3_gs::device::*;
use sdl3_gs::event::{Event, SDL_Scancode, WindowEventKind};
use sdl3_gs::sys::gpu;
...
impl MsaaTargets {
    fn create(device: &Device, width: u32, height: u32, format: SDL_GPUTextureFormat) -> Self {
        let msaa = device
            .create_texture(&SDL_GPUTextureCreateInfo {
...
impl Renderer {
    pub fn new(device: &Device) -> Self {
        ...
        let mut fragment_shader = device
            .create_shader(&ShaderCreateInfo {
        ...
        let vertex_buffer = device
            .create_buffer(SDL_GPUBufferUsageFlags::VERTEX, vertex_data_size)
            .expect("Failed to create vertex buffer");
        vertex_buffer
            .upload(None, 0, bytemuck::cast_slice(&QUAD_VERTICES))
            .expect("Failed to upload vertex data");

        // Index buffer
        let index_data_size = std::mem::size_of_val(&QUAD_INDICES) as u32;
        let index_buffer = device
            .create_buffer(SDL_GPUBufferUsageFlags::INDEX, index_data_size)
            .expect("Failed to create index buffer");
        index_buffer
            .upload(None, 0, bytemuck::cast_slice(&QUAD_INDICES))
        ...
        let checkerboard_texture = device
            .create_texture(&gpu::SDL_GPUTextureCreateInfo {
                r#type: SDL_GPUTextureType::_2D,
